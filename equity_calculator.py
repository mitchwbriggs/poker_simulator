...
class Simulation:
    ...
    def __init__(self, scenario):
        ...
        with open('poker_hash_lookup.pickle', 'rb') as handle:
    ...
    def run(self):
        ...
        num_primes = num_missing_board + (num_opponents * 2)
        for _ in range(num_rounds):
            count += 1

            rndm_primes = random.sample(avbl_primes,num_primes)
            board_primes_full = board_primes + rndm_primes[:num_missing_board]
            user_score = self.get_score(user_primes, board_primes_full)

            num_beaten = 0
            num_tied = 0
            start_index = num_missing_board
        ...
        win_pct = str(round(total_wins / count * 100)) + '%'
        return win_pct
...
class Scenario:
    ...
    def __init__(self, user, opponents, board_cards):
        self.user = user # Player object (defined in holdem_engine.py)
        self.opponents = opponents # list of Player objects (defined in holdem_engine.py)
